...
import { FileText, Home, MapPin, User, Calendar, Edit, Loader2, FileJson, Link as LinkIcon } from "lucide-react";
import { updateServiceOrderEmployees, getServiceOrderById } from "@/app/actions/soActions";
        ...
        <BreadcrumbList>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink href={`/municipios/${serviceOrder.municipality_id}`}>
              <MapPin className="h-4 w-4 mr-1" />{serviceOrder.municipalities?.name}
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbLink href="#">OS {serviceOrder.number}</BreadcrumbLink></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold">OS {serviceOrder.number}</h1>
        ...
        <div className="flex gap-2">
          ...
          <Button><LinkIcon className="mr-2 h-4 w-4" /> Relatório</Button>
        </div>
      </div>
                   ...
                   <p className="font-medium">Licitação Referente</p>
                    <Button variant="link" className="p-0 h-auto text-[#EC610D]" asChild>
                      <Link href={`/municipios/${serviceOrder.municipality_id}/licitacoes/${serviceOrder.bid_id}`}>
                        {serviceOrder.bids.number} - {serviceOrder.bids.object}
                      </Link>
                    </Button>
                 </div>
               </div>
          ...
          <CardContent className="space-y-4">
            {serviceOrder.service_order_employees.length > 0 ? (
              serviceOrder.service_order_employees.map(({ profiles: employee }: { profiles: any }) => (
                employee && <div key={employee.id} className="flex items-center gap-4">
                  <Avatar>
